...
if (isset($_GET['w']) && strlen($_GET['w'])>0){	
	$w = " WHERE TBL_APROVACOES_SEGURANCA.ID_KAIZEN =" .$_GET['w'];
}else{
	$w = "";
}
...
$sql = "SELECT 
TBL_APROVACOES_SEGURANCA.ID_APROVACAO_SEGURANCA,
TBL_APROVACOES_SEGURANCA.ID_USUARIO, TBL_APROVACOES_SEGURANCA.ID_KAIZEN, 
DATE_FORMAT(TBL_APROVACOES_SEGURANCA.DT_APROVACAO,'%d/%m/%Y') AS 'DT_APROVACAO',
...
FROM TBL_APROVACOES_SEGURANCA
left join USUARIOS.TBL_USUARIOS on TBL_APROVACOES_SEGURANCA.ID_USUARIO = USUARIOS.TBL_USUARIOS.ID_USUARIO
